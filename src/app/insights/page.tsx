...
export default function Insights() {
  ...
  // Check if user is JV Coordinator
  const isJVCoordinator = userData?.role === "jv_coordinator";
  ...
  const askAI = async () => {
    ...
    try {
      ...
      const context = {
        ...
        summary: {
          total_volume: productionData.reduce(
            (sum, e) => sum + e.gross_volume_bbl,
            0
          ),
          avg_bsw:
            productionData.reduce((sum, e) => sum + e.bsw_percent, 0) /
            productionData.length,
          avg_temp:
            productionData.reduce((sum, e) => sum + e.temperature_degF, 0) /
            productionData.length,
        },
    ...
    <div className={`min-h-screen ${COLORS.background.gradient} pt-20`}>
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          {/* Date Range Selector */}
          <div
            className={`${COLORS.background.card} backdrop-blur-xl ${COLORS.border.light} border rounded-2xl p-6 mt-6`}
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-4">
              {[
                {
                  value: "current_month" as DateRangePreset,
                  label: "Current Month",
                },
                { value: "last_month" as DateRangePreset, label: "Last Month" },
                {
                  value: "last_2_months" as DateRangePreset,
                  label: "Last 2 Months",
                },
                {
                  value: "last_6_months" as DateRangePreset,
                  label: "Last 6 Months",
                },
            ...
            <div
              className={`mt-4 p-3 ${COLORS.background.glass} rounded-xl flex items-center justify-between`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div
              className={`${COLORS.background.card} backdrop-blur-xl ${COLORS.border.light} border rounded-xl p-4`}
            >
          ...
          <div className="p-6 border-b border-white/10">
            <div className="flex space-x-1">
              {tabs.map((tab) => {
                ...
                const isLocked = tab.restricted && !isJVCoordinator;
                return (
                  <button
                    key={tab.id}
                    ...
                    className={`flex cursor-pointer items-center space-x-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 ${
                      activeTab === tab.id
                        ? `bg-gradient-to-r ${COLORS.primary.blue[600]} ${COLORS.primary.purple[600]} text-white shadow-lg`
                        : isLocked
                        ? `${COLORS.text.muted} opacity-50 cursor-not-allowed`
                        : `${COLORS.text.secondary} hover:${COLORS.text.primary} hover:${COLORS.background.glassHover}`
                    }`}
                    title={isLocked ? "JV Coordinator access only" : undefined}
                  >
            ...
            {activeTab === "query" && (
              <div className="space-y-4">
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 ${COLORS.background.glass} backdrop-blur-sm ${COLORS.border.light} border rounded-xl ${COLORS.text.primary} placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                ...
                {!isJVCoordinator ? (
                  ...
                  >
                    ...
                    <p className={`${COLORS.text.secondary}`}>
                      This feature is restricted to JV Coordinators only.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    {!predictions ? (
                      <div
                        className={`p-8 text-center ${COLORS.background.glass} rounded-xl`}
                      >
                        <TrendingUp
                          className={`w-16 h-16 ${COLORS.text.muted} mx-auto mb-4`}
                        />
                      ...
                      <div className="space-y-4">
                        ...
                        {predictions.methodology && (
                          ...
                          >
                            <h3
                              className={`font-semibold ${COLORS.text.primary} mb-2`}
                            >
                              Methodology
                            </h3>
                ...
                {!isJVCoordinator ? (
                  ...
                  <>
                    ...
                    {!insights ? (
                      ...
                      <div
                        className={`p-6 ${COLORS.background.glass} ${COLORS.text.secondary} rounded-xl ${COLORS.border.light} border prose prose-invert max-w-none markdown-content`}
                      >
